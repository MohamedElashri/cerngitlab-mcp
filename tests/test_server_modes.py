...
class TestModeDetection:
    ...
    def test_detect_mode_both_host_and_port(self):
        ...
        with patch.dict(os.environ, {
            'CERNGITLAB_HOST': '0.0.0.0',
            'CERNGITLAB_PORT': '9000'
        }):
            assert detect_mode() == 'http'
...
class TestServerCLI:
    ...
    def test_main_http_mode_explicit(self, mock_run_http, runner):
        ...
        with patch('asyncio.run') as mock_asyncio_run:
            result = runner.invoke(main, ['--mode', 'http', '--host', 'localhost', '--port', '8000'])
    ...
    @patch('cerngitlab_mcp.server.run_stdio_server')
    def test_main_auto_mode_detects_stdio(self, mock_run_stdio, mock_detect_mode, runner):
    ...
    @patch.dict(os.environ, {'CERNGITLAB_HOST': 'example.com', 'CERNGITLAB_PORT': '9000'})
    @patch('cerngitlab_mcp.server.run_http_server')
...
class TestEntryPoints:
    ...
    @patch.dict(os.environ, {'CERNGITLAB_HOST': 'custom.host', 'CERNGITLAB_PORT': '9999'})
    @patch('cerngitlab_mcp.server.run_http_server')
