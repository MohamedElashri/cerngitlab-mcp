...
class TestGetProjectInfo:
    ...
    async def test_returns_project_details_with_languages(self, client, httpx_mock):
        httpx_mock.add_response(
            url=httpx.URL(
                "https://gitlab.example.com/api/v4/projects/atlas%2Fathena",
                params={"statistics": "true"},
            ),
            json={
                "id": 100,
                "name": "athena",
                "path_with_namespace": "atlas/athena",
        ...
        httpx_mock.add_response(
            url=httpx.URL(
                "https://gitlab.example.com/api/v4/projects/atlas%2Fathena/languages"
            ),
...
class TestGetFileContent:
    ...
    async def test_returns_text_content_with_language(self, client, httpx_mock):
        ...
        result = await get_file_content.handle(
            client, {"project": "123", "file_path": "main.py"}
        )
    ...
    async def test_detects_root_files_as_binary(self, client, httpx_mock):
        ...
        result = await get_file_content.handle(
            client, {"project": "123", "file_path": "data.root"}
        )
    ...
    async def test_cmake_gets_correct_language_hint(self, client, httpx_mock):
        httpx_mock.add_response(json={"default_branch": "main"})
        httpx_mock.add_response(
            json=make_file_response("cmake_minimum_required()", "CMakeLists.txt")
        )
        result = await get_file_content.handle(
            client, {"project": "123", "file_path": "CMakeLists.txt"}
        )
    ...
    async def test_gitlab_ci_yml_is_not_binary(self, client, httpx_mock):
        ...
        result = await get_file_content.handle(
            client, {"project": "123", "file_path": ".gitlab-ci.yml"}
        )
        assert result["is_binary"] is False
...
class TestSearchCode:
    ...
    @pytest.mark.asyncio
    async def test_global_search_without_advanced_search_returns_error(
        self, client, httpx_mock
    ):
        httpx_mock.add_response(
            status_code=400,
            json={
                "error": "Scope supported only with advanced search or exact code search"
            },
        )
        result = await search_code.handle(client, {"search_term": "RooFit"})
        assert result["total_results"] == 0
    ...
    async def test_project_search_falls_back_to_tree_grep(self, client, httpx_mock):
        ...
        result = await search_code.handle(
            client, {"search_term": "RooFit", "project": "123"}
        )
    ...
    async def test_search_code_pagination(self, client, httpx_mock):
        ...
        result = await search_code.handle(
            client, {"search_term": "test", "page": 2, "per_page": 5}
        )
...
class TestSearchLhcbStack:
    ...
    @pytest.mark.asyncio
    async def test_resolves_stack_and_delegates_to_search_code(
        ...
        httpx_mock.add_response(
            url=httpx.URL(
                "https://gitlab.example.com/api/v4/projects/lhcb%2FBoole/search",
                params={
                    "search": "test",
                    "scope": "blobs",
                    "per_page": "20",
        ...
        result = await search_lhcb_stack.handle(
            client, {"search_term": "test", "stack": "sim11", "project": "lhcb/Boole"}
        )
    ...
    @pytest.mark.asyncio
    async def test_explicit_ref_overrides_stack_ref(
        self, client, httpx_mock, monkeypatch
    ):
...
class TestSearchIssues:
    ...
    async def test_search_issues(self, client, httpx_mock):
        ...
        result = await search_issues.handle(
            client, {"search_term": "Bug", "project": "123"}
        )
...
class TestInspectProject:
    ...
    async def test_inspect_project_combines_analysis(self, client, httpx_mock):
        ...
        httpx_mock.add_response(
            url=httpx.URL(
                "https://gitlab.example.com/api/v4/projects/123/repository/files/CMakeLists.txt",
                params={"ref": "main"},
            ),
            json=make_file_response(
        ...
        deps = [
            d for d in result["dependencies"] if d["source_file"] == "CMakeLists.txt"
        ]
        assert len(deps) >= 1
    ...
    async def test_inspect_project_ci_analysis(self, client, httpx_mock):
        ...
        httpx_mock.add_response(
            url=httpx.URL(
                "https://gitlab.example.com/api/v4/projects/123/repository/files/.gitlab-ci.yml",
                params={"ref": "main"},
            ),
            json=make_file_response(
...
class TestGetRelease:
    ...
    async def test_returns_full_release_details(self, client, httpx_mock):
        ...
        result = await get_release.handle(
            client, {"project": "123", "tag_name": "v1.0"}
        )
    ...
    async def test_returns_not_found_for_missing_release(self, client, httpx_mock):
        httpx_mock.add_response(status_code=404, json={"message": "not found"})
        result = await get_release.handle(
            client, {"project": "123", "tag_name": "v999"}
        )
